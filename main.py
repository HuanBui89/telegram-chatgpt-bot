...
SUGGESTIONS = [
    ...
    "Mình có thể giúp gì cho bạn hôm nay? ❤️"
]
...
# Trả lời bằng ChatGPT với đa ngôn ngữ
def chat_with_gpt(user_id, message):
    try:
        ...
        history.append({"role": "user", "content": processed_text})
...
def reset_history(update: Update, context: CallbackContext):
    ...
    try:
        ...
        update.message.reply_text("❌ Lỗi khi reset lịch sử")
...
def draw_image(update: Update, context: CallbackContext):
    ...
    try:
        ...
        response = openai.Image.create(
            prompt=prompt + ", anime style, colorful",
            n=1,
            size="512x512"
        )
        ...
        update.message.reply_text("❌ Nội dung không phù hợp để tạo ảnh")
    except Exception as e:
        logger.error(f"Draw error: {str(e)}")
        update.message.reply_text(f"❌ Lỗi khi tạo ảnh: {str(e)}")
...
def handle_message(update: Update, context: CallbackContext):
    ...
    try:
        ...
        if len(user_text) < 2:
            message.reply_text(random.choice(SUGGESTIONS))
            return

        # Xử lý lời chào
        greetings = ["hi", "hello", "chào", "yo", "alo", "hey", "hê"]
        if user_text.lower().split()[0] in greetings:
            message.reply_text(random.choice([
        ...
        message.reply_text("⚠️ Bot bị lỗi, thử lại sau nha!", reply_to_message_id=message.message_id)
